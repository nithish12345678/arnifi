                        ...
                        <div className="home-card" key={post._id}>
                            <div className="card">
                                ...
                                <h4 className="card-title">
                                    ...
                                    <Link to={post.postedBy &&((post.postedBy._id == state._id) ? "/profile" : `/profile/${post.postedBy._id}`)}>
                                    ...
                                    {post.postedBy &&((post.postedBy._id == state._id) && <abbr title="Delete Post"><IconButton style={{ marginLeft: "auto" }} aria-label="delete" onClick={() => deletePost(post._id)}>
                                        <DeleteIcon />
                                    </IconButton> </abbr>)}
                                ...
                                <div className="card-content">
                                    <div>
                                        {/* {console.log("includes :"+post.likes.includes(state))} */}
                                        {
                                            post.likes.some(
                                                (like) => like._id && state._id&&  like._id.toString() === state._id.toString()  )?
                                    ...
                                    <h6 onClick={handleLikesClick}>{post.likes.length} likes</h6>
                                    {/* show likes */}
                                    {showlikes && <div>
                                        <h6 style={{ fontWeight: "500" }}> Liked By</h6>
                                        {
                                            post.likes.map(likedBy => {
                                                return (
                                                    <h6 key={likedBy._id}>
                                                        {console.log("id:" + likedBy._id)}


                                                        <Link to={(likedBy._id == state._id) ? "/profile" : `/profile/${likedBy._id}`}>
                                                            <div style={{ display: 'flex', alignItems: 'center' }}>
                                                                <img src={likedBy.pic} alt={likedBy.userName} style={{ width: '20px', height: '20px', borderRadius: '50%', marginRight: '10px' }} />

                                                                <span style={{ fontWeight: "500" }} >{likedBy.userName}</span>
                                                            </div>
                                                        </Link>
                                                    </h6>
